...
/**
 * Représente une stratégie de joueur robot intelligente utilisant l'algorithme de Dijkstra (BFS sans heuristique).
 *
 * @author Florian Pépin.
 * @version 1.0
 */
public class DijkstraBotStrategy implements BotStrategy {
    ...
    private static class Node implements Comparable<Node> {
        ...
        int f; // coût total (égal à g car h=0 dans Dijkstra)
        Node parent;
    ...
    public void actionStrategy(BattlefieldModel battlefieldProxy) {
        ...
        // 3. Sélection de la cible et Recherche de chemin Dijkstra
        List<int[]> path = null;
        ...
        // 4. Déplacement si un chemin optimal Dijkstra est trouvé
        if (path != null && !path.isEmpty()) {
    ...
    private List<int[]> findPath(BattlefieldModel battlefieldProxy, Entity[][] grid, Hero myHero, int targetX, int targetY) {
        ...
        openSet.add(new Node(startX, startY, 0, null));
        ...
        while (!openSet.isEmpty()) {
            ...
            for (int[] dir : directions) {
                ...
                if (battlefieldProxy.isValidPosition(nx, ny)) {
                    ...
                    Node neighbor = new Node(nx, ny, nextG, current);
