...
/**
 * Représente une stratégie de joueur robot intelligente utilisant l'algorithme Glouton (Greedy Best-First Search).
 *
...
public class GreedyBotStrategy implements BotStrategy {
    ...
    private static class Node implements Comparable<Node> {
        ...
        int f; // coût total (égal à h uniquement dans Greedy Best-First Search)
        Node parent;

        Node(int x, int y, int h, Node parent) {
            this.x = x;
            this.y = y;
            this.h = h;
            this.f = h;
    ...
    public void actionStrategy(BattlefieldProxy battlefieldProxy) {
        ...
        // 3. Sélection de la cible et Recherche de chemin Glouton
        List<int[]> path = null;
        ...
        // 4. Déplacement si un chemin Glouton est trouvé
        if (path != null && !path.isEmpty()) {
    ...
    private List<int[]> findPath(Entity[][] grid, Hero myHero, int targetX, int targetY) {
        ...
        openSet.add(new Node(startX, startY, Math.abs(startX - targetX) + Math.abs(startY - targetY), null));
        ...
        while (!openSet.isEmpty()) {
            ...
            for (int[] dir : directions) {
                ...
                if (nx >= 0 && nx < size && ny >= 0 && ny < size) {
                    ...
                    Node neighbor = new Node(nx, ny, nextH, current);

                    boolean skip = false;
                    for (Node openNode : openSet) {
                        if (openNode.x == nx && openNode.y == ny && openNode.h <= nextH) {
                            skip = true;
                            break;
                        }
                    }
